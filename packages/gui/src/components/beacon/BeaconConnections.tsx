...
import BeaconAddConnection from './BeaconAddConnection';
import BeaconCloseConnection from './BeaconCloseConnection';
...
const cols = [
  ...
  },
  {
    ...
    field(row: Connection) {
      return (
        <BeaconCloseConnection nodeId={row.nodeId}>
          {({ onClose }) => (
            <StyledIconButton onClick={onClose}>
              <DeleteIcon />
            </StyledIconButton>
          )}
        </BeaconCloseConnection>
      );
    },
  },
];
...
export default function Connections() {
  ...
  const { data: connections, isLoading } = useGetBeaconConnectionsQuery();

  function handleAddPeer() {
    openDialog(<BeaconAddConnection />);
  }

  return (
    <Card
      title={<Trans>Beacon Client Connections</Trans>}
      action={
